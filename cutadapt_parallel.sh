...
# Input directory containing .fastq.gz files (this script is a loop to run in parallel computing for the trimming of the adapters from the single end reads
...
run_cutadapt() {
    ...
    adapter_seq="" #input your adapter sequences
    adapter_seq1="" #add this for multiple adapters to be removed from the single end reads
    ...
    cutadapt -a "$adapter_seq" -a "$adapter_seq1"  -o "$output_file" "$input_file" $cutadapt_opts
...
find "$input_dir" -name "*.fastq.gz" | parallel -j 30 run_cutadapt
